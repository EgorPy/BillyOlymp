...
public class BE2_Ins_WhenKeyPressed : BE2_InstructionBase, I_BE2_Instruction
{
    ...
    protected override void OnEnableInstruction()
    {
        ...
        // if (mm.selectedObject.transform.name == TargetObject.Transform.name) {
    ...
    void OnUpdate()
    {
        ...
        // Debug.Log(mm.selectedObject.transform.name);
        if (Input.GetKeyDown(_key))
        {
            if (mm.selectedObject != null) {
                if (mm.selectedObject.transform.name == TargetObject.name) {
