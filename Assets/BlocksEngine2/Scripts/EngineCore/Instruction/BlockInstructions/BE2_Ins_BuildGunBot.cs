...
public class BE2_Ins_BuildGunBot : BE2_InstructionBase, I_BE2_Instruction
{
    int mineralsCount;
    public float waitSeconds;
    public int gunBotCost;
    ...
    GameObject BuildGunBot() {
        ...
        foreach(GameObject b in factories) {
            ...
            if (distance < factoryDistance) {
                pos = new Vector3(b.transform.position.x, 0f, b.transform.position.z) - (b.transform.forward * 14);
            }
        }
        ...
        GameObject go = Instantiate(gunBot, pos, TargetObject.Transform.rotation);
        PlayerPrefs.SetInt("mineralsCount", mineralsCount - gunBotCost);
        var objects = Resources.FindObjectsOfTypeAll<GameObject>();
        go.transform.GetChild(0).name = "GunBot " + objects.Length;
        ...
        // GameObject obj = Instantiate(gunBot, new Vector3(pos.x, 2.8f, pos.z), TargetObject.Transform.rotation);
        // // obj.transform.Translate(0, 0, gunBot.GetComponentInChildren<Renderer>().bounds.size.z);
    ...
    public new void Function() {
        if (_firstPlay)
        {
            ...
            if (canBuildVilly()) {
                // Debug.Log("true");
                if (mineralsCount >= gunBotCost) {
                    PlayerPrefs.SetInt("mineralsCount", mineralsCount - gunBotCost);
                    obj = BuildGunBot();
                    _counter = waitSeconds;
                    _firstPlay = false;
                } else {
                    error.errorMessage = $"Нехватает минералов, нужно {gunBotCost}";
                }
            } else {
                error.errorMessage = $"Нет Завода поблизости";
